...
class TbActiveForm extends CActiveForm
{
	...
	public function groupSitesControlGroup($model, $attribute, $title)
    {
	   ...
	   if(count($model->sites)>0)
	   		$checked_values = CHtml::listData($model->sites, 'id', 'id_site');
	   ...
	   foreach ($options as $id => $option)
	   {
		   if(isset($checked_values))
		   		$checked = ( array_search($id, $checked_values) ? 1 : 0 );
			else $checked  = 0;
		   
		   $html .= "<li>";
		   $class_model_id = $model_name."[".$attribute."][]";
		   $html .= CHtml::label($option, "$model_name_$id");
		   $html .= CHtml::checkBox($class_model_id, $checked, array('value'=>$id, 'id'=>"$model_name_$id"));
		   $html .= "</li>";
		  // '<input class="span8" maxlength="255" name="Users[login]" id="Users_id_site" value="1" type="checkbox">';
